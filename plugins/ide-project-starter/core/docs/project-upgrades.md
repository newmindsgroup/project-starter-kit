...
In a Git repository, record the renames explicitly. On macOS and Windows, Git ignores letter case by default, so `git add -A` does not record a rename that changes only case: the disk has the new name while the repository keeps the old one, and a Linux checkout still sees it. Finish lists the exact commands under git_restage, one `git rm --cached` plus `git add` per record. Run them straight after finish, before any checkout, reset, pull or commit: on these systems a checkout or reset can write the old name back to disk, after which `git add` of the new name matches nothing and the file silently drops out of the repository. Confirm with `git ls-files`. The upgrade helper refuses to stage while Git still records an old name.
