...
In a Git repository, record the renames explicitly. On macOS and Windows, Git ignores letter case by default, so `git add -A` does not record a rename that changes only case: the disk has the new name while the repository keeps the old one, and a Linux checkout still sees it. Finish lists the exact commands under git_restage, one `git rm --cached` plus `git add` per record.
